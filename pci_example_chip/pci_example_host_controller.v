...
// PCI Interface uses these wires to request local memory activity.   
  input  [PCI_BUS_DATA_RANGE:0] pci_target_ref_address;
  input  [3:0] pci_target_ref_command;
  input  [PCI_BUS_CBE_RANGE:0] pci_target_byte_enables_l;
  input  [PCI_BUS_DATA_RANGE:0] pci_target_write_data;
  output [PCI_BUS_DATA_RANGE:0] pci_target_read_data;
...
// Signals used by Master test code to apply correct info to the PCI bus
  reg    [PCI_BUS_DATA_RANGE:0] hold_master_address;
...
// Grab address and data in case it takes a while to get bus mastership
      ...
      begin
        ...
        hold_master_address[PCI_BUS_DATA_RANGE:0] <= hold_master_address[PCI_BUS_DATA_RANGE:0];
        hold_master_command[3:0] <= hold_master_command[3:0];
        hold_master_data[PCI_BUS_DATA_RANGE:0] <= Update_Captured_Host_Data
               ? (hold_master_data[PCI_BUS_DATA_RANGE:0] + 32'h01010101)
               : hold_master_data[PCI_BUS_DATA_RANGE:0];
// NOTE:  A real Master might change byte enables throughout a Burst.
        hold_master_byte_enables_l[PCI_BUS_CBE_RANGE:0] <= Update_Captured_Host_Data
        ...
        modified_master_address[PCI_BUS_DATA_RANGE:0] <= modified_master_address[PCI_BUS_DATA_RANGE:0];
      end
...
// These variables come from a ficticious processor or DMA device
  ...
  wire   [PCI_BUS_DATA_RANGE:0] Host_Mem_Write_Data = hold_master_data[PCI_BUS_DATA_RANGE:0];
  wire   [PCI_BUS_CBE_RANGE:0] Host_Mem_Write_Byte_Enables_l = hold_master_byte_enables_l[PCI_BUS_CBE_RANGE:0];
...
// These following variables come from the PCI Delayed Read State Machine below.
  ...
  wire   [PCI_BUS_DATA_RANGE:0] mem_write_data = Host_Mem_Write_Request
                         ? Host_Mem_Write_Data[PCI_BUS_DATA_RANGE:0]
                         : PCI_Bus_Mem_Write_Data[PCI_BUS_DATA_RANGE:0];
  wire   [7:2] mem_address = (Host_Mem_Read_Request | Host_Mem_Write_Request)
                         ? Host_Mem_Address[7:2] : PCI_Bus_Mem_Address[7:2];
  wire   [PCI_BUS_CBE_RANGE:0] mem_write_byte_enables_l = Host_Mem_Write_Request
                         ? Host_Mem_Write_Byte_Enables_l[PCI_BUS_CBE_RANGE:0]
                         : PCI_Bus_Mem_Write_Byte_Enables_l[PCI_BUS_CBE_RANGE:0];
  ...
  reg    [PCI_BUS_DATA_RANGE:0] mem_read_data;
...
`ifdef VERILOGGER_BUG
    ...
    mem_read_data[PCI_BUS_DATA_RANGE:0] <= mem_read_enable
      ...
      begin
        $display ("*** %m %h - Local Memory Read Data invalid 'h%h, expected 'h%h, at %t",
                    ...
                    {Example_Host_Mem_3[mem_address[7:2]],
                     ...
                     Example_Host_Mem_1[mem_address[7:2]],
                     Example_Host_Mem_0[mem_address[7:2]]},
                     hold_master_data[PCI_BUS_DATA_RANGE:0], $time);
        error_detected <= ~error_detected;
      end
      `NO_ELSE;
    end
...
`ifdef VERBOSE_TEST_DEVICE
    ...
    begin
      $display ("Example Host Controller %h - Local Memory written with Address %h, Data %h, Strobes %h, at time %t",
                 test_device_id[2:0], {mem_address[7:2], 2'b00},
                 mem_write_data[`PCI_BUS_DATA_RANGE:0],
                 mem_write_byte_enables_l[PCI_BUS_CBE_RANGE:0], $time);
    end
    `NO_ELSE;
`endif  // VERBOSE_TEST_DEVICE
`else  // VERILOGGER_BUG
...
//   can be marked finished immediately, by a combinational path.
  ...
  begin
    ...
    begin
      ...
      pci_master_updated_data[PCI_BUS_DATA_RANGE:0] <= hold_master_data[PCI_BUS_DATA_RANGE:0];  // don't care
    ...
    begin  // Issue local Memory Reference
      ...
      host_doing_write <= host_doing_write;  // don't care
      pci_master_updated_data[PCI_BUS_DATA_RANGE:0] <= pci_master_updated_data[PCI_BUS_DATA_RANGE:0];  // don't care
      pci_master_updated_burst_size[3:0] <= pci_master_updated_burst_size[3:0];  // don't care
    end
    ...
    begin  // must be an instantaneous Config Write ack
      ...
      pci_master_updated_data[PCI_BUS_DATA_RANGE:0] <= hold_master_data[PCI_BUS_DATA_RANGE:0];
    ...
    begin
      ...
      begin
        ...
        pci_master_updated_data[PCI_BUS_DATA_RANGE:0] <= pci_master_updated_data[PCI_BUS_DATA_RANGE:0];
      ...
      begin
        ...
        begin
          host_command_finished <= 1'b0;
          host_working_on_pci_reference <= 1'b1;
          if (pci_master_read_data[PCI_BUS_DATA_RANGE:0] !== pci_master_updated_data[PCI_BUS_DATA_RANGE:0])
          begin
            $display ("*** %m %h - Read Data %h not expected %h, at %t",
                        test_device_id[2:0], pci_master_read_data[PCI_BUS_DATA_RANGE:0],
                        pci_master_updated_data[PCI_BUS_DATA_RANGE:0], $time);
            error_detected <= ~error_detected;
          end
        end
        host_doing_write <= host_doing_write;
...
// This would be fine of the Host had a write buffer.
  assign  pci_master_ref_address[PCI_BUS_DATA_RANGE:0] = modified_master_address[PCI_BUS_DATA_RANGE:0];
  ...
  assign  pci_master_byte_enables_l[PCI_BUS_CBE_RANGE:0] = hold_master_byte_enables_l[PCI_BUS_CBE_RANGE:0];
  assign  pci_master_write_data[PCI_BUS_DATA_RANGE:0] = ~host_working_on_pci_reference
                                 ? hold_master_data[PCI_BUS_DATA_RANGE:0]
                                 : pci_master_updated_data[PCI_BUS_DATA_RANGE:0];
...
//   no performance loss.
  ...
  reg    [PCI_BUS_DATA_RANGE:0] pci_target_hold_write_data;
  reg    [PCI_BUS_CBE_RANGE:0] pci_target_hold_byte_enables_l;
...
// NOTE WORKING put in access to SRAM here
  ...
  begin
    ...
    begin
      ...
      pci_target_hold_write_data[PCI_BUS_DATA_RANGE:0] <=
                            pci_target_hold_write_data[PCI_BUS_DATA_RANGE:0]; 
      pci_target_hold_byte_enables_l[PCI_BUS_CBE_RANGE:0] <=
                            pci_target_hold_byte_enables_l[PCI_BUS_CBE_RANGE:0];
    ...
    begin
      ...
      begin
        ...
        pci_target_hold_write_data[PCI_BUS_DATA_RANGE:0] <=
                        pci_target_write_data[PCI_BUS_DATA_RANGE:0]; 
        pci_target_hold_byte_enables_l[PCI_BUS_CBE_RANGE:0] <=
                        pci_target_byte_enables_l[PCI_BUS_CBE_RANGE:0];
  ...
  assign  PCI_Bus_Mem_Write_Data[PCI_BUS_DATA_RANGE:0] = ~mem_ref_in_progress
                                   ? pci_target_write_data[PCI_BUS_DATA_RANGE:0]
                                   : pci_target_hold_write_data[PCI_BUS_DATA_RANGE:0];
  assign  PCI_Bus_Mem_Write_Byte_Enables_l[PCI_BUS_CBE_RANGE:0] = ~mem_ref_in_progress
                                   ? pci_target_byte_enables_l[PCI_BUS_CBE_RANGE:0]
                                   : pci_target_hold_byte_enables_l[PCI_BUS_CBE_RANGE:0];
  ...
  assign  pci_target_read_data[PCI_BUS_DATA_RANGE:0] = mem_read_data[PCI_BUS_DATA_RANGE:0];
